...
export class EvaluationAttachmentComponent implements OnInit, OnDestroy {
    ...
    evaluationAttachments: EvaluationAttachment[];
    currentAccount: any;
    ...
    loadAll() {
        this.evaluationAttachmentService.query({
            page: this.page,
    ...
    trackId(index: number, item: EvaluationAttachment) {
        return item.id;
    }
    registerChangeInEvaluationAttachments() {
        this.eventSubscriber = this.eventManager.subscribe('evaluationAttachmentListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.evaluationAttachments.push(data[i]);
        }
    }
